...
当前已实现的仓库命令见[开发指南](./docs/development.zh-CN.md)。`cargo xtask check`、`fmt`、`clippy`、`test`、`test-core`、`test-format`、`test-plan`、`test-node <id>`、`test-material <id>`、`golden check`、受保护的 `golden update <id> --accept`、`doc`、`deps`、`links`、`shader-check` 和显式 `gpu-smoke` 目前可用。CLI 已实现纯 CPU `validate` 和 `inspect --plan`、支持 `--skip-probe` 的验证型 `doctor` 及图 `render`／`render-builtin checker`，均提供人类可读与 JSON 模式。下面其余命令是目标接口，不得声称已经实现。
