...
基线核对于 2026-09-22：M6-A、M6-B 和 NUM-01 已集成。[发布状态](./docs/release.zh-CN.md)负责当前发布及平台验收声明，源码清单负责构建版本。工作中的 PERF-MAT 候选为未发布且尚未验收的 Rust 0.8.0 / browser 0.8.0-alpha.0；最近已验收的 MAT-01 基线为记录的软件与 GT 1030 范围内的 0.6；记录的浏览器发布版为 0.3.0-alpha.0。集成、验收、发布是不同状态，不得把历史里程碑计划当作当前待办。

- **版本边界：** `.mix` 保持 v1，`RenderPlan` 与浏览器 API schema 为 v3。计划哈希域为 `mixture-render-plan-v3\0`，包含选中外部图像的身份。图 `render` 和 `inspect --plan` 报告为 schema 3；doctor、固定 checker、asset 外层报告为 schema 1；`validate` 保持无版本外层结构。见[兼容性](./docs/compatibility.zh-CN.md)。
...
[MAT-02a 选定契约](./docs/mat-02-layered-weathering.zh-CN.md)为有界涂漆金属用例选定 scalar-morphology@1 与 scalar-subtract@1，两个身份已通过 PR #56–57 集成；节点证据不接受完整材质。[减法夹具](./fixtures/nodes/scalar-subtract/README.zh-CN.md)验证必需 Scalar 输入、饱和端点及精确半精度可表示差值，使用一个保留全零的 16 字节 uniform；Core 负责契约和降级，wgpu 负责唯一像素操作。[记录的减法证据](./docs/evidence/mat-02-subtract-node/README.zh-CN.md)在 Windows Vulkan／DX12 各有 64 组 Native／浏览器精确一致用例；PR #57 已以 cbeb367261bf09b3b7acd2540b4efbee3000e7e7 合并，合并前后六项检查均通过（见[集成摘要](./docs/perf-mat-texture-reuse.zh-CN.md)）。[节点夹具](./fixtures/nodes/scalar-morphology/README.zh-CN.md)覆盖类型验证、原始半精度极值、周期支撑集合、退化尺寸、轴顺序交换及半径单调性；[记录的 Windows 公开消费证据](./docs/evidence/mat-02-morphology/README.zh-CN.md)在 Vulkan／DX12 各有 192 组 Native／浏览器精确一致用例；PR #56 已通过六项必需检查，并以 313074451cd6ddb5e5f82cce933c3d4fe3b4ed38 合并；PR #57 的组合 main 矩阵也已通过全部六项检查。Core 负责语义／降级，wgpu 负责唯一内核。[冻结设计与计划](./fixtures/materials/painted-metal/README.zh-CN.md)规定调用方控制、计划 23 pass、七预设／四尺寸／五通道、精确探针场／ABI，以及不变的 4/255 降采样、24 pass 和 512 MiB 目标。[既有输入可行性](./docs/evidence/mat-02-input-feasibility/README.zh-CN.md)及[减法反例](./docs/evidence/mat-02-subtraction/README.zh-CN.md)支持设计，不验收新节点。[真实 PERF-MAT 进入证据](./docs/evidence/perf-mat-before/README.zh-CN.md)保留 2K 拒绝（805,306,832 > 536,870,912 字节）及有效的 23 pass／五通道 1K 渲染。[ADR 0009](./docs/decisions/0009-transient-texture-reuse.zh-CN.md)选定 Core 的确定性最后使用／物理槽规划和 wgpu 的单次渲染复用，不改变内核或 pass 顺序。[工作中的复用实现](./docs/perf-mat-texture-reuse.zh-CN.md)一起更新 Core 估算和 wgpu 槽，选定 plan／hash／API／图报告 v3 及未发布 0.8，保持 .mix／.mixpack v1 和预算不变。重新编译源请求，使计划／哈希缓存失效。完整干净候选像素／成本／生命周期／公开消费验收及六项检查仍为必需。精确的十一用例 v2→v3 哈希迁移保持内核及像素 golden；trace-2k 现验证物理槽计费。不引入全局池或着色器变更。首次节点实现因内核穷举变化选定 Rust 0.7.0／browser 0.7.0-alpha.0，下游穷举匹配须处理 ScalarMorphology 与 ScalarSubtract。不改变格式、自动迁移、唯一执行器或发布规则。契约已通过 [PR #55](https://github.com/OpenMixture/OpenMixture/pull/55) 集成，保留六项门槛及配对文档。

MAT-02 [夹具请求构建器](./fixtures/materials/painted-metal/README.zh-CN.md)现已校验冻结的调用方控制值，并从未改变的实测图生成完整的 28 项请求矩阵。这属于引擎验收工具，不新增 Core API、格式或节点语义，不涉及迁移，也不代表材质已验收。`node --test scripts/painted-metal-requests.test.mjs` 覆盖映射与拒绝规则，并在两个浏览器工作流中执行。公开 Native／浏览器矩阵候选现已检查 28 项请求／140 项通道对照、重复／包／切片等价、所有权、内存及 release 耗时。候选 SDK 运行要求 21 项测试，registry 仍为 13 项。Chromium 执行 `check-painted.mjs`，维持 <=1/255 门槛及图／请求／包身份校验；原始半精度参数因果／接缝、PBR／人工门槛仍未完成。工具不代表材质通过验收。
...
crates/mixture-core/
  ...
  src/compiler/allocation.rs  确定性最后使用与物理槽规划
