...
[MAT-02 前期设计](./docs/mat-02-layered-weathering.zh-CN.md)是草案，不是已准入节点或已接受材质。它提议由 Core／wgpu 负责的有界轴向 Scalar 形态处理及饱和减法，并复用既有组合。MAT-01 退出条件已满足；MAT-02a 目录／版本／验收计划接受仍是实现前置条件。[具体 MAT-02a 图与矩阵](./fixtures/materials/painted-metal/README.zh-CN.md)仍为设计评审输入：计划 23 pass、显式调用方控制映射、每组配对 140 次比较。静态保留全部纹理的估算超过不变的 512 MiB 目标；先冻结可行性，再以实际图测量启动 PERF-MAT。本设计产物不准入新节点身份。[减法反例](./docs/evidence/mat-02-subtraction/README.zh-CN.md)记录干净源码中既有节点的组合限制，不是新节点验收。本次准备不改变运行时、格式或迁移规则；检查配对文档／链接，并保留六项既有门槛。
