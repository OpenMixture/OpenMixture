...
MAT-02 [夹具请求构建器](./fixtures/materials/painted-metal/README.zh-CN.md)现已校验冻结的调用方控制值，并从未改变的实测图生成完整的 28 项请求矩阵。这属于引擎验收工具，不新增 Core API、格式或节点语义，不涉及迁移，也不代表材质已验收。`node --test scripts/painted-metal-requests.test.mjs` 覆盖映射与拒绝规则，并在两个浏览器工作流中执行。完整材质门槛仍未完成。
