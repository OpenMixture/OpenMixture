# material-output v1 夹具

[English](./README.md) | 简体中文

PR-007 的 [input.mix](./input.mix) 和 [cases.json](./cases.json)覆盖 `material-output` 图契约。用例：`defaults`, `aliases`。清单包含 2 个有效 GPU 用例和 1 个无效源文件／覆盖用例。样本为固定预期 RGBA8 哨点，显式使用 0／1 码值容差，不通过 CPU 图求值器生成。无效用例必须在获取 GPU 前失败。
