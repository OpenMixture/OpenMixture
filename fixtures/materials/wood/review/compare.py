...
def main():
    ...
    cases = (
        ("default", "01  WOOD-LIKE CANDIDATE", "repeat 32 / warp 0.018 / orientation 0"),
        ("coarse-grain", "02  COARSER GRAIN", "repeat 16 / warp 0.018 / orientation 0"),
        ("straight-grain", "03  STRAIGHT GRAIN", "repeat 32 / warp 0 / orientation 0"),
        ("horizontal-grain", "04  HORIZONTAL GRAIN", "repeat 32 / warp 0.018 / orientation 1"),
    )
    if (report.get("schemaVersion") != 1
            or report.get("kind") != "external-pbr-appearance-review"
            or report.get("material") != "wood"
            or set(report.get("results", {})) != {case for case, _, _ in cases}):
        raise ValueError("Expected a completed wood review with all four cases")
    ...
    draw.text((margin, 22), "WOOD-LIKE / MATERIAL REVIEW", font=title, fill="#f2f4f8")
