...
这些是 [MAT-02 契约](../../../docs/mat-02-layered-weathering.zh-CN.md)的设计输入，不是已实现材质或已接受结果。[图设计](./graph-design.json)明确**不是 `.mix`**：两个新节点身份尚未实现，`$` 值是调用方替换项，不是新的运行时表达式语言。[验收计划](./qualification-plan.json)指定七个预设 × 四种尺寸 × 五通道（每组 Native／浏览器配对 140 次比较），但在列出的前置工作解决前保持 `mat-02a-review-before-freeze`。不得作为材质夹具运行，也不得把数值目标当作测量结果。
...
冻结 MAT-02a 之前，须用既有节点输入和规定半径映射确认默认降采样目标的可行性，评审精确探针场与解析判定，并记录目录／版本选择。既有节点可行性不验收尚未实现的形态处理或完整材质。准入、实现、验收及发布继续分别记录。
