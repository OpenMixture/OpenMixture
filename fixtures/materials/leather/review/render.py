"""Optional Blender consumer of verified leather PNGs; never executes a .mix graph."""
import argparse
import hashlib
...
CASES = ("detail-min", "default", "detail-max", "coarse-grain")
...
def verify_inputs(expected):
    ...
    if manifest["material"] != "leather" or set(manifest["files"]) != required:
        raise ValueError("Expected the leather manifest with all sixteen PNGs")
