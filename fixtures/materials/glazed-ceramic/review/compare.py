...
def main():
    ...
    cases = (
        ("default", "01  GLAZED CANDIDATE", "8 x 8 tiles / roughness 43/255"),
        ("matte", "02  MATTE CONTROL", "Same color / roughness 112/255"),
        ("fine-tiles", "03  FINER PATTERN", "16 x 16 tiles / roughness 43/255"),
    )
    cell, margin, gutter, top = 480, 24, 12, 112
    sheet = Image.new("RGB", (2 * margin + 3 * cell + 2 * gutter, 714), "#10151c")
    draw = ImageDraw.Draw(sheet)
    title = ImageFont.load_default(size=30)
    heading = ImageFont.load_default(size=21)
    body = ImageFont.load_default(size=17)
    draw.text((margin, 22), "GLAZED CERAMIC / MATERIAL REVIEW", font=title, fill="#f2f4f8")
    ...
    draw.text((margin, 679), "Sphere and backing are display geometry. Exported normal is neutral; exported height is zero.", font=body, fill="#96a5b8")
    sheet.save(output)
