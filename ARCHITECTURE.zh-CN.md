...
### 3.5 未来的 `mixture-wasm`

只有到 M5 才能添加 `mixture-wasm`。
...
[M5 计划](./M5_PRS.zh-CN.md)选择单一浏览器分发包，暂名 `@openmixture/runtime`，在本仓库由同一次 JS 接口、声明及 WASM 构建生成。独立产品仓库消费该包，先实现 Player，后实现 Studio。产品控件、请求新鲜度、预览、文件导出及编辑器布局仍归消费者。不引入独立 SDK 仓库或额外语义执行器。

[拟议浏览器 SDK 契约](./docs/browser-sdk.zh-CN.md)定义显式 WASM／GPU 初始化、无需 GPU 的验证／目录访问、异步完成、自有 RGBA8 输出及销毁。浏览器适配必须保持原生语义，并适配平台等待和错误交付；把原生路径编译成 WASM 不代表浏览器验收。这些是 M5 要求，不是已实现绑定，也不改变原生 API 契约。
