...
[MAT-02 前期设计](./docs/mat-02-layered-weathering.zh-CN.md)记录邻域形态处理缺口、提议契约、组合关系及进入阶段时须冻结的决策。其中节点身份、候选版本及预算都是提议，不构成目录准入或实现授权。MAT-01 仍为活动增量；MAT-02 实现仍须等待其退出并接受 MAT-02a。
