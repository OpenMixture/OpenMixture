...
2026-09-23 的规划决定将材质表达广度选为下一方向。目标是可复用的程序化纹理材质，不是完整 Substance 3D 套件对等。保持一个主要功能增量。MAT-01 优先；后续阶段是有顺序的规划承诺，不是新节点或 schema 的全面授权。分别跟踪契约接受、实现、像素验收、集成和发布。本计划不指定包版本或发布日期。

基线已有十三种节点类型、图像输入、公开参数、八个材质输出通道及可移植资产，但缺少以下用例所需的结构构造和处理词汇。通道插槽不等于通道生成器，可移植归档不等于可复用子图。已有陶瓷／皮革／木材验收继续保持有界且必需。

| 阶段／进入条件 | 有界目标与候选能力 | 退出标准／明确排除 |
|---|---|---|
| MAT-01 — 结构；下一增量 | 一个参数化砖墙／铺地砖材质族：形状／轮廓生成、错行重复、显式种子的逐元素变化、灰缝遮罩、边缘软化及共享高度／颜色／粗糙度结构。优先最小可复用原语，不建设通用 Tile Sampler。 | 独立控制元素数量／长宽比、灰缝宽度、行偏移、高度／颜色变化及边缘软度；改变种子保持布局控制。验收默认、规则、错行及随机变化用例。不含通用散布、任意区域分割或完整形状库。 |
...
| 工作项 | 交付物 | 依赖／完成条件 |
|---|---|---|
| MAT-01a — 契约与验收设计 | [选定契约](./docs/mat-01-structured-materials.zh-CN.md)及[冻结矩阵](./fixtures/materials/brick-paving/acceptance.json)规定 brick-pattern@1、scalar-mask-blend@1、参数、坐标、种子、采样、精度和预算。 | 设计已记录；两个节点实现与材质夹具现为候选，不代表材质像素已接受。格式／架构变更须单独决策。 |
