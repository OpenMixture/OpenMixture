...
| 下一项待批准目标 | 优先评估 M6-A 最小用例：外部线性高度图与程序化 Scalar 场混合后生成高度／法线。实施前须批准格式、资源引用、尺寸、数值、所有权、预算、执行身份及版本契约。此处不批准资源 API 或节点实现。 |
| 当前执行范围 | PLAN-01 仅收敛路线图状态与中英文说明。尚无获批执行的下一项主要功能；M6-A 仍处于待决策阶段。 |
| 条件式候选 | M6-B 打包、原生分发、图复用和 GPU 互操作分别按明确问题决定。性能与数值维护由测量或回归驱动，不构成必须串行完成的功能链。 |

下一项实施计划应在用例批准后拆分，并分别记录计划、实现及验收证据。Studio 的升级、部署及产品验收不是引擎规划或发布的前置条件。
...
本次规划收敛不启动这两个方向。资源缓存、去重、增量上传和旧格式转换另需测量或明确问题。不授权内置 URL 下载器、市场、通用资源管理器或在运行时文档中加入编辑器状态。
