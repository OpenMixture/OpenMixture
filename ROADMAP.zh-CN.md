...
**当前集成状态：** M6-B 与 NUM-01 及其前置 PR #39–45 已合入 main，组合后的六项检查全部通过。未发布的 0.6 候选已在[记录范围](./docs/evidence/mat-01/README.zh-CN.md)内通过 MAT-01 验收。MAT-01 节点与验收工具已通过 PR #50–52 集成（[集成记录](./docs/evidence/mat-01/integration/README.zh-CN.md)）；人工视觉接受与合并后检查均已完成，未发布源码清单为 0.6.0 / 0.6.0-alpha.0；已发布浏览器版本为 0.3.0-alpha.0。详见[发布状态与硬件范围](./docs/release.zh-CN.md)。
...
| 规划状态 | 当前内容 |
|---|---|
| 当前可用基线 | 已发布 `@openmixture/runtime@0.3.0-alpha.0`：外部图像资源、API schema 2、计划 v2，`.mix v1` 不变。当前 Rust 0.6.0 源码可消费；MAT-01 已在记录范围内验收，crate 尚未发布。[精确归档与注册表验收](./docs/evidence/npm-030-alpha/README.zh-CN.md)。 |
| 当前增量 | MAT-02a 分层风化契约与冻结验收计划准备。MAT-01 节点、夹具和工具已集成并在[记录范围](./docs/evidence/mat-01/README.zh-CN.md)内接受。MAT-02 运行时实现仍须先接受自身契约／目录／版本。发布继续独立处理。 |
...
2026-09-23 的规划决定将材质表达广度选为下一方向。目标是可复用的程序化纹理材质，不是完整 Substance 3D 套件对等。保持一个主要功能增量。MAT-01 已在记录范围内接受，下一步为 MAT-02a 契约；后续阶段是有顺序的规划承诺，不是新节点或 schema 的全面授权。分别跟踪契约接受、实现、像素验收、集成和发布。本计划不指定包版本或发布日期。
...
[MAT-02a 选定契约](./docs/mat-02-layered-weathering.zh-CN.md)冻结形态处理／减法身份、调用方图／控制映射及[验收计划](./fixtures/materials/painted-metal/qualification-plan.json)，由绑定源码的既有输入可行性支持。首次实现选定未发布 0.7 候选，当前清单与可执行目录不变。MAT-01 退出条件已满足。契约集成后，MAT-02b 通过独立 PR 实现两个节点；预计的 2K 内存失败须先实测再启动 PERF-MAT，不放宽冻结预算。
