...
**M6B-02 已选定设计：** [格式、Rust 所有权和内存预算](./docs/m6b-package-format.zh-CN.md)已由 ADR 0008 明确，包含替代方案测量与畸形输入语料。下一项是 **M6B-03 共享 CPU 装载/写入**；设计完成不表示运行时 codec 可用或已发布。
...
| 当前增量 | M6-B：M6B-01 进入及 M6B-02 格式/所有权/预算设计已记录；下一项依据 ADR 0008 实现 M6B-03 共享 CPU 装载/写入，适配与验收随后独立推进。 |
