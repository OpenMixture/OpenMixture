...
| 当前执行范围 | [M6A-02 Core 实现](./docs/m6a-02-core-resources.zh-CN.md)交付资源验证、快照和计划身份；以本实现 PR 的检查为集成门槛。下一项 M6A-03 接通 Native 上传；浏览器资源及像素资格仍待后续任务。 |
...
两个方向保持独立。M6A-01 选定外部输入设计供集成；M6-B 仍是未排期候选。本次规划变更不实现这两个方向。
