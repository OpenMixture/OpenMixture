...
[审查认领计划](./docs/browser-alpha.zh-CN.md)将 ALPHA-06／07 交付记录为完成，明确 ALPHA-08 支持／精度文档及条件性的 ALPHA-09 消费者缺陷响应。剩余可追踪试用部署和外部用户反馈由 Studio 负责。新的引擎实现须有具体的当前契约失败或消费者需求；数值验收通过 ADR 0006 调整，并非采用已关闭的 warp 实验完成修复。
