...
[审查认领计划](./docs/browser-alpha.zh-CN.md)将 ALPHA-06／07 交付证据记录为完成，明确 ALPHA-08 支持／精度文档及上游 issue 触发的 ALPHA-09 消费者缺陷分诊。Studio 自行管理升级、产品验收、部署和用户反馈，这些不属于引擎任务。遵循 [agent 项目边界](./AGENTS.zh-CN.md)：引擎缺陷在本仓库复现和修复，通过上游 issue 反馈已验证修复／版本，由 Studio 验收产品升级。本地发现的引擎回归和引擎检查失败也可触发工作。目前未安排新的引擎实现；数值验收通过 ADR 0006 调整，并非采用已关闭的 warp 实验完成修复。
...
以下段落保留原有日期与当时的下一步判断；当前状态与认领顺序以上方交付收尾计划为准。
