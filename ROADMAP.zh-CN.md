...
下一目标是可独立使用和验证的 SDK，随后交付一个有界材质表达增量。保持一个主要功能增量加必要维护的节奏。本批按 ENG-01、ENG-02 的顺序落实文档及规则／测试变更。ENG-03／04 属于后续计划，尚未实现或验收，需要单独实施。
...
| ENG-04 — 一个材质表达增量 | [用例与拟议契约](./docs/eng-04-scalar-blend.zh-CN.md)已设计：两个 Scalar 高度场组合后生成法线。实现及像素验收仍待完成，不要求先完成全部宿主迁移。 | 先批准具体用例及输入／权重／范围／精度／版本契约，再增加最小 scalar-blend。Native／浏览器执行、边界／平铺／因果测试及已有材质回归通过。 |
