...
| 当前验收 | **M6A-05 验收，2026-09-21：** [留存验收](./docs/evidence/m6a-05/README.zh-CN.md)关闭记录的 Linux 软件矩阵内综合验收：八组资源通道逐字节一致，Scalar、三材质回归和六项必需检查通过。Windows 硬件一致性仍失败，不属于已验收范围；≤1 门槛不变。0.3.0 Rust 源码 / 0.3.0-alpha.0、API schema 2 浏览器候选均未发布。实现 PR 集成和发布仍是独立动作。 |
...
两个方向保持独立。M6-A 已在功能分支栈实现，并于有记录的软件矩阵内验收；M6-B 仍为未排期候选。

| 方向 | 进入决策 | 有界目标 |
|---|---|---|
| M6-A — 外部图像输入 | M6A-01 选定[外部高度合同](./docs/m6a-resource-contract.zh-CN.md)供设计集成。实现按 M6A-02–05 推进，不要求容器或 Studio 资源面板。 | 调用方提供资源；引擎验证身份、尺寸、格式、预算，上传并管理 GPU 生命周期。网络／文件／权限归调用方；CLI 解码属于适配层。 |
| M6-B — 可移植资产打包 | 证明普通 .mix 加外部文件不足以解决分发／加载问题。独立于 M6-A 决策。 | 定义可检查资源身份、有界加载和路径安全；有依据时才考虑 .mixpack。 |

本次合同决策不实现资源或启动可移植打包。资源缓存、去重、增量上传和旧格式转换另需测量或明确问题。不授权内置 URL 下载器、市场、通用资源管理器或在运行时文档中加入编辑器状态。
