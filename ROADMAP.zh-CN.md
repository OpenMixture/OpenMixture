...
| 工作项 | 交付物 | 依赖／完成条件 |
|---|---|---|
| MAT-01a — 契约与验收设计 | 冻结砖墙／铺地砖参考目标、参数用例、最小原语及明确类型／版本目录提案。规定端口、范围、种子、平铺／坐标规则、滤波、精度、分辨率和内存边界。在渲染验收候选前指定像素数值容差和性能预算。 | 下一可执行任务。先审查 Core 契约、WGSL 所有权和兼容性；接受设计不等于像素已实现。格式／架构变更须单独决策。 |
