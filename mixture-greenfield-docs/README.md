...
Mixture reads a versioned `.mix` material document, validates and compiles its directed acyclic graph, executes the resulting compute passes through one `wgpu` renderer, and returns requested PBR texture channels.
...
The first format is intentionally human-readable and editor-agnostic:
...
The repository is intended to be dual-licensed under either Apache-2.0 or MIT. The foundation pull request must add both license files before the project is advertised or distributed.
