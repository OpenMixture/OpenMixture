...
The package is pre-alpha and publication remains disabled; the workspace manifest owns its version. Document version remains 1 and plan version is 2; the reviewed node catalog is listed in the repository's [node contracts](https://github.com/OpenMixture/OpenMixture/blob/main/docs/node-contracts.md). Image upload/execution is owned by mixture-wgpu. Reject unsupported inputs explicitly. See the repository's [compatibility policy](https://github.com/OpenMixture/OpenMixture/blob/main/docs/compatibility.md) and [local package verification](https://github.com/OpenMixture/OpenMixture/blob/main/docs/package-consumption.md).
...
M6A-02 adds Core image preparation and plan v2. GPU image execution belongs to the implemented M6A-03 wgpu upload path. See the [implementation boundary](https://github.com/OpenMixture/OpenMixture/blob/main/docs/m6a-02-core-resources.md).
