...
The browser compilation and transport boundary for OpenMixture M5. This unpublished crate calls `mixture-core` for strict raw-source decoding, validation, catalog and immutable compilation, and calls `mixture-wgpu` for explicit browser WebGPU acquisition and asynchronous rendering. Package inspection and rendering reuse the shared `mixture-asset` codec. It introduces no pixel code or product state.
