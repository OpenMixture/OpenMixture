...
OpenMixture M5 的浏览器编译和传输边界。此未发布 crate 调用 `mixture-core` 完成严格原始源码解析、校验、目录和不可变计划编译，调用 `mixture-wgpu` 完成显式浏览器 WebGPU 获取和异步渲染。包检查与渲染复用共享的 `mixture-asset` 编解码器。不增加像素代码或产品状态。
