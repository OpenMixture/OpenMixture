...
本地 0.2.0 包仍处于 pre-alpha，禁用发布。它需要匹配版本的 `mixture-core`，并通过已文档化的高级句柄及 limits 暴露 wgpu 30 类型。设备丢失不触发恢复；OOM 根据类型化错误分类。调度和 CPU 输出聚合保留由调用者负责。见仓库[兼容性策略](https://github.com/OpenMixture/OpenMixture/blob/main/docs/compatibility.zh-CN.md)。
