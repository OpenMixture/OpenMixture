...
`AssetError` preserves original typed Core errors or a structured `PackageDiagnostic` with stable code, `package` stage and evidence. No extraction, compression, external resource lookup, fallback or implicit cache exists. Source version 0.5.0 remains unpublished; CLI/browser package adapters are separate work.
