...
| Active increment | MAT-02b morphology and saturating-subtraction implementations under the integrated frozen contract (PR #55). MAT-01 nodes, fixture and tools are integrated and accepted within [recorded scope](./docs/evidence/mat-01/README.md). Both nodes are integrated through PRs #56–57 with recorded node evidence and six passing combined main checks; full material qualification remains pending. Publication remains separate. |
...
[MAT-02a selected contract](./docs/mat-02-layered-weathering.md) freezes the morphology/subtraction identities, caller graph/control mappings and [qualification plan](./fixtures/materials/painted-metal/qualification-plan.json), supported by source-bound existing-input feasibility. It selects an unpublished 0.7 candidate at first implementation; the integrated PR #56–57 node implementations select 0.7 and add both reviewed identities; full material qualification remains pending. MAT-01 exit is satisfied. After contract integration, MAT-02b implements each node through a separate PR; the [actual 2K failure and valid 1K baseline](./docs/evidence/perf-mat-before/README.md) now activate PERF-MAT without relaxing the frozen budget. [ADR 0009](./docs/decisions/0009-transient-texture-reuse.md) selects the bounded allocation/version change: PERF-MATa retains evidence and contract; PERF-MATb implements Core scheduling, wgpu reuse and thin version projections together; PERF-MATc qualifies pixels, costs, lifecycle and public consumers. The [working Core/wgpu implementation](./docs/perf-mat-texture-reuse.md) now selects plan/API v3 and the unpublished 0.8 candidate; complete source-bound qualification is pending. MAT-02 remains the single active material increment; its full material gates and MAT-03/04 remain required.
...
MAT-02 recipe revision 2 is an explicit candidate correction for measured height-offset precision loss; see the [relative-height contract](./docs/mat-02-relative-height.md). It preserves revision 1 evidence and all existing acceptance gates. This does not close MAT-02 or advance MAT-03/04.
