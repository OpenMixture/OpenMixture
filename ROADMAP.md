...
|---|---|---|
| MAT-01a — contract and acceptance design | Freeze a brick/paving reference brief, parameter cases, minimum primitives and exact type/version catalog proposal. Specify ports, ranges, seeds, tile/coordinate rules, filtering, precision, resolution and memory bounds. Assign numeric image tolerances and performance budgets before rendering acceptance candidates. | Next actionable task. Review Core contracts, WGSL ownership and compatibility first; accepted design is not implemented pixels. Any format/architecture change needs its own decision. |
