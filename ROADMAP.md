...
**Implementation status:** PR-001 through PR-007 are implemented. The M2 six-node graph path, three readable examples, pipeline cache, and requested PNG outputs pass local Metal and pinned SwiftShader checks; see [graph execution evidence](./docs/graph-rendering.md). M0/M1 remote clean-checkout and Linux SwiftShader gates remain open until CI actually runs. Local M2 verification does not close those gates. At the user’s request, PR-008 proceeds while remote CI acceptance is deferred: protected golden tooling and 1K checker machine gates are implemented; controlled PBR evidence now supplements the flat channel previews, and the user has accepted the ceramic target against that evidence. See [material acceptance](./docs/material-goldens.md). PR-009 now implements the three noise/color/normal nodes and the 1K [leather candidate](./fixtures/materials/leather/README.md), including machine checks and controlled PBR evidence; the user has accepted the leather appearance review. PR-010 now adds the two resampling nodes, directional wood and measured 2K allocation evidence. The user has accepted wood. The [six-question M3 review](./docs/m3-review.md) is complete, with local acceptance, release latency and bounded 2K evidence. The [M4 implementation train](./M4_PRS.md) has begun: PR-011 verifies the [public Rust consumer](./docs/native-sdk.md) on both local adapter policies, with owned outputs and 1K release measurements. PR-012 verifies [CLI reports, exit codes and partial writes](./docs/cli-contract.md), and repairs omitted human diagnostic context. PR-013 adds [typed GPU failures and context-loss tracking](./docs/gpu-failures.md), with real destruction and cleanup checks. PR-014 verifies [latest-result publication and bounded retention](./docs/stale-results.md). PR-015 now verifies [actual local package consumption](./docs/package-consumption.md). The [M4 exit assessment](./docs/release.md) records local acceptance and compatibility limits. Deferred remote gates remain open and block a release-ready claim; the next train requires an explicit decision.

**Remote verification update, 2026-09-12:** [CI is active](./docs/evidence/remote-ci/README.md). The earlier deferral is superseded: all three CPU platforms pass, while the latest Linux GPU failure remains under repair. M5 is not started.
