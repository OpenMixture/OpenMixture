...
MAT-02b `tests/subtract.rs` uses only public Core compilation and wgpu rendering for 64 direct/amplified cases. Set `MIXTURE_SUBTRACT_BROWSER` to the browser receipt and `MIXTURE_SUBTRACT_EVIDENCE` to a fresh output file. Prefer the [subtraction comparison wrapper](../../fixtures/nodes/scalar-subtract/README.md), which binds the exact candidate and preserves the explicit target directory. This test is GPU-only and ignored by ordinary CPU checks.
