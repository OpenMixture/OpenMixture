...
MAT-02b candidate mode now requires 18 tests, including 192 periodic morphology cases and exact repeat checks in `morphology.spec.mjs`. The test is explicitly included only for candidate qualification through `MIXTURE_MORPHOLOGY_TESTS=1`; exact published registry consumption remains 13 tests. The [morphology fixture guide](../../fixtures/nodes/scalar-morphology/README.md) defines the independent Native comparison command. This adds engine qualification, not a Studio upgrade or material acceptance.
