...
test('scalar composition executes in the candidate and fails explicitly in the published runtime', async ({ page, browser }, testInfo) => {
  ...
  await openHost(page);
  const fixture = ['0.6.0-alpha.0', '0.8.0-alpha.0'].includes(expectedBuild.runtimeVersion) ? 'scalar-blend-v2.mix' : 'scalar-blend.mix';
  ...
  if(expectedBuild.runtimeVersion==='0.1.0-alpha.0') {
    ...
    expect(['0.2.0-alpha.0','0.3.0-alpha.0','0.6.0-alpha.0','0.8.0-alpha.0']).toContain(expectedBuild.runtimeVersion);
