...
[M6A-02](./docs/m6a-02-core-resources.zh-CN.md)增加第十三种 Core 节点合同 image-input、不可变资源准备和计划 v2；GPU 图像上传仍待 M6A-03。Rust 源码为 0.3.0，浏览器候选为 0.3.0-alpha.0／API schema 2，均未发布；精确已发布 npm 版本仍为 0.2.0-alpha.0。
...
无需 GPU 即可验证[棋盘格文档](./examples/checker.mix)：`cargo run --locked -p mixture-cli -- validate examples/checker.mix --json`。参阅[严格文件格式](./docs/file-format.zh-CN.md)和[节点契约](./docs/node-contracts.zh-CN.md)。
...
M3 已接受以下三种材质；目录按[节点准入规则](./ARCHITECTURE.zh-CN.md#72-经审查的节点目录与准入)演进时，其证据和回归门槛继续必需：
