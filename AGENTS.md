...
Baseline reviewed 2026-09-22: M6-A, M6-B and NUM-01 are integrated. [Release status](./docs/release.md) owns current publication and qualified-platform claims; source manifests own build versions. The integrated MAT-01 candidate is Rust 0.6.0 / browser 0.6.0-alpha.0, unpublished and qualified for MAT-01 within the recorded software and GT 1030 scope; the recorded published browser package is 0.3.0-alpha.0. Integration, qualification and publication are distinct. Do not treat historical milestone plans as the current backlog.
...
The [material capability roadmap](./ROADMAP.md) records MAT-01 brick/paving acceptance and selects MAT-02a layered-weathering contract work next, followed by MAT-03 woven surfaces and MAT-04 graph reuse. Later capabilities remain planned, not implemented catalog entries. Freeze each bounded contract, catalog/version review and acceptance cases before implementation; add only primitives justified by the selected material. PERF-MAT starts from measured cost failures. This sequence addresses expression gaps while retaining one active feature increment.
...
The [MAT-02 preparatory design](./docs/mat-02-layered-weathering.md) is a draft, not an admitted node or accepted material. It proposes bounded axial Scalar morphology and saturating subtraction owned by Core/wgpu and reuses existing composition. MAT-01 exit is satisfied; MAT-02a catalog/version/qualification-plan acceptance remains a prerequisite to implementation. The [concrete MAT-02a graph and matrix](./fixtures/materials/painted-metal/README.md) remain design-review inputs: 23 planned passes, explicit caller control mapping and 140 comparisons per pairing. Its static retain-all texture estimate exceeds the unchanged 512 MiB target; freeze feasibility first and measure the actual graph before PERF-MAT. No new node identity is admitted by this design artifact. The [subtraction counterexample](./docs/evidence/mat-02-subtraction/README.md) records a clean-source existing-node limitation, not new-node qualification. No runtime, format or migration rule changes in this preparation; check paired documentation/links and preserve the existing six gates.
