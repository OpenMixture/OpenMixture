...
Core owns semantics/lowering, wgpu owns pixels, and adapters stay thin; Studio work remains separately owned. This planning change does not change formats, node behavior, package versions or migration policy. Each stage requires the roadmap's multi-resolution, parameter-causality, seam, PBR visual and public Native/browser evidence, existing material regressions and all six required checks. Preserve historical failures and qualify only measured hardware. Publication stays separate.
