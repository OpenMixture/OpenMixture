...
The [MAT-02 preparatory design](./docs/mat-02-layered-weathering.md) is a draft, not an admitted node or accepted material. It proposes bounded axial Scalar morphology and saturating subtraction owned by Core/wgpu and reuses existing composition. MAT-01 exit is satisfied; MAT-02a catalog/version/qualification-plan acceptance remains a prerequisite to implementation. The [subtraction counterexample](./docs/evidence/mat-02-subtraction/README.md) records a clean-source existing-node limitation, not new-node qualification. No runtime, format or migration rule changes in this preparation; check paired documentation/links and preserve the existing six gates.
