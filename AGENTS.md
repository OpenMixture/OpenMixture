...
The working [gamma-one levels correction](./docs/levels-linear-correction.md) bypasses approximate `pow(t,1)` in the sole wgpu shader to evaluate the existing linear identity. The deliberate compatibility decision retains `levels@1`, its Core/ABI/plan contracts and hashes; build-bound pixels may change. No migration or golden reset. Exact midpoint fixtures, old material regressions and full Native/browser qualification are required; source 76e8039 is qualified within the [recorded software/GT 1030 scope](./docs/evidence/levels-linear-correction/README.md); integration and full MAT-02 acceptance remain separate.
...
The MAT-02 [fixture request builder](./fixtures/materials/painted-metal/README.md) now validates frozen caller controls and emits the complete 28-request matrix from the unchanged measured graph. This is engine-owned qualification tooling, not a new Core API, format or node behavior; no migration or acceptance claim. `node --test scripts/painted-metal-requests.test.mjs` covers mapping/rejection and runs in both browser workflows. The public Native/browser matrix candidates now check 28 requests/140 channel comparisons, repeat/package/sliced equivalence, ownership, memory and release timing. Candidate SDK runs require 21 tests; registry remains 13. Chromium runs `check-painted.mjs` with unchanged <=1/255 tolerance and source/request/package identities; raw-half causality/seams and PBR/human gates remain pending. No material acceptance follows from the tooling.
