...
[ADR 0009](./decisions/0009-transient-texture-reuse.md) is implemented by the working [0.8 physical-slot candidate](./perf-mat-texture-reuse.md), pending full qualification. Plan/hash/API and graph reports use v3; recompile .mix requests and invalidate stored plan hashes. Source .mix and package v1 remain unchanged.
...
| Nodes | Seventeen built-in types, fifteen pixel kernels in the working 0.8 candidate; `fractal-noise` supports explicit versions `1` and `2`, other nodes require `1`. IDs, ports, parameter types/ranges/defaults and explicit seeds come from core. | Semantic changes require deliberate node/format version decisions, contracts, fixtures and golden evidence; no duplicate catalog or silent default change. |
| RenderPlan | Plan version `3`; immutable compiled semantics and deterministic canonical ordering. | Review serialization and exact hash snapshots. A changed compiler representation or semantic input may invalidate saved plans/hashes; `.mix` remains the source of truth. |
| CLI reports | Schema `3` for `inspect --plan` and graph `render`; schema `1` for doctor, fixed checker and asset envelopes; `validate` retains its original unversioned `{ok, diagnostics}` envelope. | Preserve each command's presence/null/omission rules and exits; update independent process tests and docs with deliberate wire changes. Do not invent a version field for existing validate output. |
