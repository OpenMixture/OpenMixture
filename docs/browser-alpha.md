...
| ALPHA-04 — Publishable npm Alpha candidate | Next delivery task | Produce a clean versioned archive, digest/build identity, release notes and support scope. Hand the exact candidate to Studio for saved-file/Player/export upgrade acceptance. Registry publication is a separate action, followed by independent installation of that exact version. |
