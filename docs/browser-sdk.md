...
The unpublished 0.8 candidate uses API/plan schema 3. Plans expose Core-owned `allocation` (`slots`, `resourceSlots`, `lastUses`); estimates add bigint `textureCount` and `logicalTextureBytes`. Slot IDs and last-use indices are u32 numbers. See [texture reuse and migration](./perf-mat-texture-reuse.md); historical published 0.3 remains API/plan 2.
...
## M6A-04 image requests

`RenderRequest.resources` accepts `ImageBinding[]`; `resourceLimits` accepts a partial bigint policy. `validate`, `inspect` and `render` share this request type. See [browser resources](./m6a-04-browser-resources.md) for synchronous capture, offset views, errors, ownership, reproduction and version boundaries.
