...
空间 Scalar 混合由独立的 `scalar-mask-blend@1` 提供，下方统一权重契约不变。
...
必需 Scalar 输入 `a`、`b`、`mask`，Scalar `value` 输出。Float `opacity` 在 [0,1]，默认 1。限制有限样本范围，遮罩乘以 opacity 后选择精确端点或插值，不重采样。完整规则见 [MAT-01 契约](./mat-01-structured-materials.zh-CN.md)及[字面夹具](../fixtures/nodes/scalar-mask-blend/README.zh-CN.md)。已有 scalar-blend 不增加输入或改变行为。
