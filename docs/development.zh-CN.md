...
仓库实现了[实施计划](../INITIAL_PRS.zh-CN.md)中的 PR-001 至 PR-003。
它包含三个产品 crate 边界和私有仓库工具。核心提供[诊断与安全限制 API](./diagnostics.zh-CN.md)；[显式 GPU 获取与 doctor](./gpu-context.zh-CN.md)已可用。材质解析和计算／回读尚未实现。所有软件包均禁用发布。
...
PR-003 唯一允许的直接依赖关系如下，包括构建、开发、可选和特定目标依赖：

| 软件包 | 允许的依赖 |
| --- | --- |
| `mixture-core` | 运行时使用 `serde`；`serde_json` 仅用作开发依赖 |
| `mixture-wgpu` | 工作区内的 `mixture-core`、`wgpu`、`serde`；仅开发时使用 `pollster`、`serde_json` |
| `mixture-cli` | 工作区内的 `mixture-core`、`mixture-wgpu`、`pollster`、`serde_json` |
| `xtask` | `pulldown-cmark`、`serde_json` |

核心使用 `serde` 序列化强类型诊断和限制；仅在开发时使用 `serde_json` 编码测试快照和公共示例。工具使用 `pulldown-cmark` 解析 Markdown，使用 `serde_json` 读取 Cargo 元数据。只有 `mixture-wgpu` 直接依赖 `wgpu`，其 `serde` 用于编码能力报告。`pollster` 在 CLI／测试边界驱动异步获取，`serde_json` 用于 CLI 报告和测试断言。核心仍不依赖 GPU。原生后端特性策略见 [GPU 指南](./gpu-context.zh-CN.md)。全部已解析依赖版本记录在 [Cargo.lock](../Cargo.lock) 中。
...
可执行程序名为 `mixture`。帮助／版本返回 `0`。`doctor` 获取真实上下文，成功返回 `0` 和 `unverified`，获取失败返回 `1` 和 `unhealthy` 报告。JSON 模式向 stdout 写入一份报告，人类可读模式包含相同的策略和能力证据。无效选项、缺少命令，以及尚未实现的 `validate`／`inspect`／`render` 返回 `2`，说明写入 stderr。输出 I/O 失败返回 `1`。见 [doctor 用法](./gpu-context.zh-CN.md)和[共享退出码策略](./diagnostics.zh-CN.md)。

节点、着色器、执行计划、材质和基准相关 xtask 命令尚未实现，调用会返回错误。引入顺序见[初始 PR 实施计划](../INITIAL_PRS.zh-CN.md)。
...
[独立 GPU CI](../.github/workflows/gpu-smoke.yml)构建固定 SwiftShader Vulkan 适配器并运行获取冒烟测试，远端结果仍待获取。不声称计算或回读健康状态已验证。

智能体指南中尚未实现的运行时模块仍是未来职责地图。当前编译根模块为 [core](../crates/mixture-core/src/lib.rs)、[wgpu](../crates/mixture-wgpu/src/lib.rs)、[CLI](../crates/mixture-cli/src/main.rs) 和 [xtask](../xtask/src/main.rs)。空的夹具和示例目录包含范围说明，没有伪造材质资产。
