...
fn main() {
    ...
    let inputs: Vec<f32> = if args.len() > 4 { std::fs::read(&args[4]).unwrap().chunks_exact(4).map(|x| f32::from_le_bytes(x.try_into().unwrap())).collect() } else { vec![0.0_f32; count] };
    assert_eq!(inputs.len(), count);
    let context = pollster::block_on(GpuContext::request(mixture_wgpu::GpuContextOptions { backend: if std::env::var("MIXTURE_GPU_BACKEND").as_deref()==Ok("vulkan") { mixture_wgpu::BackendPreference::Vulkan } else { mixture_wgpu::BackendPreference::Dx12 }, ..Default::default() })).unwrap();
    ...
    #[cfg(windows)] {
        ...
        let command = format!("Get-Process -Id {} | Select-Object -ExpandProperty Modules | Where-Object ModuleName -eq 'dxcompiler.dll' | Select-Object FileName,ModuleName | ConvertTo-Json", std::process::id());
        let modules = std::process::Command::new("powershell.exe").args(["-NoProfile", "-Command", &command]).creation_flags(0x08000000).output().unwrap();
