...
try {
 ...
 result.cases.push({case:c.case,size:c.size,actualRgba:actual});read.unmap();read.destroy();u.destroy();textures.forEach(t=>t.destroy());
 }
 const err=await d.popErrorScope();if(err)throw Error(err.message);return result;
 }finally{d.destroy();}
 },{cases,code});writeFileSync(out,JSON.stringify(report,null,2));console.log(JSON.stringify({cases:report.cases.length}));
