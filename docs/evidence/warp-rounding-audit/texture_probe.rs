...
fn main(){
 ...
 let context=pollster::block_on(GpuContext::request(GpuContextOptions{backend:if std::env::var("MIXTURE_GPU_BACKEND").as_deref()==Ok("vulkan"){BackendPreference::Vulkan}else{BackendPreference::Dx12},..Default::default()})).unwrap();
 ...
 let width=node["size"][0].as_u64().unwrap() as u32; let height=node["size"][1].as_u64().unwrap() as u32; let pitch=(width*8).div_ceil(256)*256;
 let texture=device.create_texture(&wgpu::TextureDescriptor{label:None,size:wgpu::Extent3d{width,height,depth_or_array_layers:1},mip_level_count:1,sample_count:1,dimension:wgpu::TextureDimension::D2,format:wgpu::TextureFormat::Rgba16Float,usage:wgpu::TextureUsages::STORAGE_BINDING|wgpu::TextureUsages::TEXTURE_BINDING|wgpu::TextureUsages::COPY_SRC,view_formats:&[]});
 ...
 let staging=device.create_buffer(&wgpu::BufferDescriptor{label:None,size:(pitch*height) as u64,usage:wgpu::BufferUsages::COPY_DST|wgpu::BufferUsages::MAP_READ,mapped_at_creation:false});
 let mut encoder=device.create_command_encoder(&Default::default());{let mut pass=encoder.begin_compute_pass(&Default::default());pass.set_pipeline(&pipeline);pass.set_bind_group(0,&group,&[]);pass.dispatch_workgroups(width.div_ceil(8),height.div_ceil(8),1);}
 encoder.copy_texture_to_buffer(texture.as_image_copy(),wgpu::TexelCopyBufferInfo{buffer:&staging,layout:wgpu::TexelCopyBufferLayout{offset:0,bytes_per_row:Some(pitch),rows_per_image:Some(height)}},wgpu::Extent3d{width,height,depth_or_array_layers:1}); queue.submit([encoder.finish()]);
 let(tx,rx)=std::sync::mpsc::channel();staging.slice(..).map_async(wgpu::MapMode::Read,move|r|tx.send(r).unwrap());device.poll(wgpu::PollType::Wait{submission_index:None,timeout:Some(std::time::Duration::from_secs(60))}).unwrap();rx.recv().unwrap().unwrap();std::fs::write(format!("{}/{}.bin",args[2],node["id"].as_str().unwrap()),&staging.slice(..).get_mapped_range().unwrap().chunks(pitch as usize).flat_map(|row| row[..(width*8) as usize].to_vec()).collect::<Vec<u8>>()).unwrap();staging.unmap();textures.push(texture);
 }
}
