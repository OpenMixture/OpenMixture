...
fn main() {
    ...
    for node in nodes.as_array().unwrap() {
        let width = node["size"][0].as_u64().unwrap() as u32;
        let height = node["size"][1].as_u64().unwrap() as u32;
        ...
        let uniform = device.create_buffer(&wgpu::BufferDescriptor {
            label: None,
            size: (node["params"].as_array().unwrap().len() * 4) as u64,
            usage: wgpu::BufferUsages::UNIFORM,
            mapped_at_creation: true,
        });
        let bytes: Vec<u8> = node["params"]
            .as_array()
            .unwrap()
            .iter()
            .flat_map(|v| (v.as_u64().unwrap() as u32).to_le_bytes())
            .collect();
        uniform
            .slice(..)
        ...
        let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: None,
            source: wgpu::ShaderSource::Wgsl(node["code"].as_str().unwrap().into()),
        });
        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: None,
            layout: None,
            module: &module,
            entry_point: node["entry"].as_str(),
            compilation_options: Default::default(),
            cache: None,
        });
        let view = texture.create_view(&Default::default());
        let inputs: Vec<_> = node["inputs"]
            .as_array()
            .unwrap()
            .iter()
            .map(|i| textures[i.as_u64().unwrap() as usize].create_view(&Default::default()))
            .collect();
        let mut entries = vec![
            wgpu::BindGroupEntry {
                binding: 0,
        ...
        std::fs::write(
            format!("{}/{}.bin", args[2], node["id"].as_str().unwrap()),
            &staging
                .slice(..)
                .get_mapped_range()
                .unwrap()
                .chunks(pitch as usize)
                .flat_map(|row| row[..(width * 8) as usize].to_vec())
                .collect::<Vec<u8>>(),
        )
        .unwrap();
        staging.unmap();
