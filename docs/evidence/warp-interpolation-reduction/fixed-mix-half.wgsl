...
@compute @workgroup_size(64) fn probe(@builtin(global_invocation_id) id:vec3<u32>){let o=id.x*32u;if o>=arrayLength(&inputs){return;}let delta=(2.0*inputs[o]-1.0)*inputs[o+1u]*inputs[o+2u];let t=fract(delta);outputs[o]=mixture_half(mix(inputs[o+3u],inputs[o+4u],inputs[o+9u]));}