...
fn cellular_noise(uv: vec2<f32>, period: u32, seed: u32) -> f32 {
    ...
    return clamp((second - nearest) / (sqrt(second) + sqrt(nearest)), 0.0, 1.0);
}
 ...
 let configs=array<vec4<u32>,6>(vec4<u32>(1024u,1024u,271828u,64u),vec4<u32>(129u,65u,4294967295u,128u),vec4<u32>(129u,65u,0u,1u),vec4<u32>(129u,65u,271828u,128u),vec4<u32>(2048u,2048u,1u,128u),vec4<u32>(1024u,1024u,4294967295u,128u));let octaves=array<u32,6>(3u,6u,1u,6u,6u,6u);let persist=array<f32,6>(0.35000000,1.00000000,0.00000000,0.00000000,1.00000000,0.01000000);let config=configs[case_id];
 ...
 let uv=vec2<f32>(inputs[sample_index*2u],inputs[sample_index*2u+1u]);
 var sum=0.0;var weights=0.0;var weight=1.0;var period=config.w;
