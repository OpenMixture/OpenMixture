...
fn value_noise(uv: vec2<f32>, period: u32, seed: u32, start:u32) -> f32 {
    let p = uv * f32(period);
    ...
    outputs[start]=t.x;outputs[start+1u]=t.y;outputs[start+2u]=fade.x;outputs[start+3u]=fade.y;outputs[start+4u]=a;outputs[start+5u]=b;outputs[start+6u]=c;outputs[start+7u]=d;outputs[start+8u]=mix(a,b,fade.x);outputs[start+9u]=mix(c,d,fade.x);return mix(mix(a,b,fade.x),mix(c,d,fade.x),fade.y);
}
fn cellular_noise(uv: vec2<f32>, period: u32, seed: u32, start:u32) -> f32 {
    let p = uv * f32(period);
