...
原生 M4／M4.1、有界 M5 和记录范围内的 Studio MVP 已完成。[npm Alpha 与精确注册表消费者](./evidence/npm-alpha/README.zh-CN.md)已交付，Rust crate 仍未发布。本页保留首次交付范围与限制。[Post-Alpha 路线图](../ROADMAP.zh-CN.md)负责当前优先级；ENG-01／02 不实现 M6 或新节点。
...
| ALPHA-02 — 状态对齐 | 已更新 | 本页保留首次交付状态，路线图负责当前工作。历史运行记录只描述自身来源及环境。 |
...
已完成的 M4／M5 和首次 Alpha 记录保留为历史验收，新候选仍需当前资格验证。保留已有必需检查，为批准的引擎用例及缺陷扩展验证。路线图中的 ENG-03／04 尚属计划，本次收尾更新不实现它们。M6、子图、全量 3D 预览、GPU 零拷贝、通用优化、市场与协作仍未排期；第二像素后端仍被禁止。产品改进属于产品自身规划。
...
引擎工作可以来自批准的里程碑、维护者定义的用例、测量、回归或外部 issue。OpenMixture 独立决定范围与验收，Studio 交付不是前置条件。ALPHA-09 继续作为外部缺陷分诊路径，不是唯一工作来源。issue 或本收尾记录不自动授权无关范围。

## 研究收口

PR #13–#18 作为未采用研究关闭。实验不改变交付着色器，也不再是发布前置条件。不能仅为不同浏览器末位对齐而引入编译策略、FMA 改写或新金图。后续数值工作需要批准的引擎用例、实测限制或具体契约失败，提供独立预期；行为变化时明确决定语义版本及兼容性。
