...
PR-005 引入首个可执行的 `.mix` 源文件格式：UTF-8 JSON，文档版本为 `1`，节点版本独立且必须显式为 `1`。此前不存在已实现的格式或迁移。本次添加解码、六个节点契约、图验证和 CLI 验证器；图编译与执行仍属于 PR-006／PR-007。
...
节点 ID 和公开参数 ID 必须匹配 `[A-Za-z][A-Za-z0-9_-]{0,63}`。节点 ID 与公开 ID 属于不同命名空间，各自在其命名空间内唯一。类型、端口和参数 ID 必须精确匹配[六个节点契约](./node-contracts.zh-CN.md)，区分大小写。上述字段使用固定拼写。
...
5. 必须恰好有一个 `material-output`，其 `baseColor` 需要有效的 `Color` 连接；其他通道使用已记录的默认值。`levels` 和 `blend` 的必填输入同样需要有效连接。
...
输出尺寸、请求通道、GPU 临时内存估算、暴露参数覆盖值的应用、依赖裁剪和 RenderPlan 生成属于后续编译请求；它们不是文档字段，也不是 `validate` 已实现的能力。
...
`to_json` 为有效源文档生成确定性的格式化 JSON：节点按 ID 字典序排列，边按完整的源／目标端点元组排列，公开绑定按 ID 排列，参数对象内的键按字典序排列。输入省略的参数对象和暴露数组会显式写为 `{}` 和 `[]`。显式值、有意义的数组顺序及源语义得到保留，不插入参数默认值。这是结构序列化，不是后续规范化计划哈希。JSON 数值参数使用现有 `serde_json` 依赖的 `float_roundtrip` 特性，使有限 f64 值在序列化／解码后保持位值一致；原始十进制拼写与空白不保留。
