...
PR-005 引入首个可执行的 `.mix` 源文件格式：UTF-8 JSON，文档版本为 `1`，节点版本独立且必须显式为 `1`。此前不存在已实现的格式或迁移。本次添加解码、六个节点契约、图验证和 CLI 验证器；PR-006 [图编译](./render-plan.zh-CN.md)现已实现，图执行仍属于 PR-007。
