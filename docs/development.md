...
At PR-004 the only allowed direct dependency edges, including build, dev, optional, and target-specific dependencies, are:
...
Node, plan, material, and golden xtask commands are not implemented yet and return an error. Follow [INITIAL_PRS.md](../INITIAL_PRS.md) for their introduction order.
...
Unimplemented runtime modules in the agent guide remain a future ownership map. Current compilation roots are [core](../crates/mixture-core/src/lib.rs), [wgpu](../crates/mixture-wgpu/src/lib.rs), [CLI](../crates/mixture-cli/src/main.rs), and [xtask](../xtask/src/main.rs). The checker has a real GPU-generated fixture; graph/material example directories still contain scope readmes.
