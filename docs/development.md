...
It contains three product crate boundaries and private repository tooling. Product crates expose no material or GPU runtime API yet. All packages have publication disabled.
...
2. The M0 dependency policy against `cargo metadata`.
...
At M0 the only allowed direct dependency edges, including build, dev, optional, and target-specific dependencies, are:
...
There are no third-party product dependencies. The tooling uses `pulldown-cmark` to parse Markdown and `serde_json` to read Cargo metadata; neither provides runtime document or rendering behavior. Transitive tooling versions are recorded in [Cargo.lock](../Cargo.lock).
...
The executable is named `mixture`. Only help and version return success. Missing arguments, unknown commands, and planned runtime commands return exit code 2 with a development-stage explanation on stderr. No fake `doctor`, `validate`, `inspect`, or `render` result is returned. Structured runtime diagnostics are introduced in PR-002 and later CLI work.

Node, shader, plan, material, golden, and GPU xtask commands are not implemented in M0 and return an error. Follow [INITIAL_PRS.md](../INITIAL_PRS.md) for their introduction order.
