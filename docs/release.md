...
**Current status (2026-09-20):** Native M4/M4.1, bounded M5 and the recorded Studio MVP are accepted. Ordinary Windows Chrome/Edge/Firefox each pass the [v2 material matrix](./evidence/browser-quality-v2/README.md) for the recorded existing archive. Packages remain unpublished. [Alpha closeout](./browser-alpha.md) now covers exact-candidate delivery, Studio upgrade acceptance and the browser required-check decision; M6 does not start.
