...
**Working PERF-MAT candidate (2026-09-23):** Source manifests select unpublished Rust 0.8.0 / browser 0.8.0-alpha.0 for [per-render texture reuse](./perf-mat-texture-reuse.md). Core and wgpu implement the same physical-slot schedule; plan/hash/API and graph-report schema are 3. The catalog remains seventeen types/fifteen kernels; .mix/.mixpack v1, node semantics and pixel goldens stay unchanged. Consumers recompile source requests and invalidate plan/hash caches. Full clean-candidate qualification and painted-metal acceptance remain pending; the last qualified MAT-01 baseline is 0.6. Published browser 0.3 remains separate. Prior 0.7 node evidence retains its original source/archive identity.
