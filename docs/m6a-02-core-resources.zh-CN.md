...
既有金图清单绑定 v1 计划哈希，因此 [v2 迁移表](./plan-v2-migration.json)单独记录十一种材质用例的精确旧／新哈希。`node scripts/verify-plan-v2.mjs` 在构建 CLI 后独立去除 v2 新增零字段、保留原数值 token，验证还原哈希与未改动的金图清单完全一致，再核验当前 v2 哈希。金图检查只接受相同 material／case／旧哈希／新哈希四元组或直接相同的哈希；任意一项变化仍失败，像素比较仍使用原金图及原阈值。这不是金图重置或一般性兼容回退。
