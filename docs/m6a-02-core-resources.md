...
Existing golden manifests bind v1 plan hashes. The separate [v2 migration table](./plan-v2-migration.json) records exact old/new hashes for eleven material cases. After building the CLI, `node scripts/verify-plan-v2.mjs` independently removes only the new zero-valued v2 fields, preserves numeric tokens, verifies the reconstructed hash against unchanged golden manifests, and verifies the current v2 hash. Golden checks accept only an exact material/case/old-hash/new-hash tuple or direct hash equality; changing any field still fails. Original pixels and tolerances remain mandatory. This is not a golden reset or a general compatibility fallback.
