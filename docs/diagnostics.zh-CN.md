...
当前 CLI 仅实现帮助／版本（`0`），并拒绝缺少参数、未知或未来命令（`2`）。运行时 JSON 分发和上述退出码映射仍由对应 CLI PR 实现。
...
核心唯一的运行时依赖是 `serde`；`serde_json` 是用于快照和示例的开发依赖。[依赖策略](./development.zh-CN.md)强制区分两者。本项工作不引入 `.mix` 格式字段、图实现、GPU 依赖、着色器、CLI 运行时命令或自动修复。M0 远端 CI 验收仍待完成；PR-002 不宣称关闭该验收项。
