...
PR-002 起初仅产生限制错误。PR-003／PR-004 已使用 GPU 代码，PR-005 使用解析／版本／节点／端口／参数／环代码。下表保留原始词汇；[PR-005 扩展](./file-format.zh-CN.md)添加字段、身份、连接数量、绑定及输入 I/O 错误。预留编译代码仍不表示编译器已实现。
...
下表适用于 PR-005 验证、PR-004 doctor／内置渲染及未来运行时命令，不代表全部命令已经实现：
...
核心现在运行时使用 `serde` 和 `serde_json`：PR-005 将现有已锁定 JSON 依赖提升为运行时依赖，用于严格解码和确定性序列化。[依赖策略](./development.zh-CN.md)继续隔离 GPU 和平台依赖。PR-002 未引入 `.mix` 格式字段、图实现、GPU 依赖、着色器、CLI 运行时命令或自动修复。GPU 获取、棋盘格执行与 doctor 另有专门文档。M0 远端 CI 验收仍待完成；PR-002 不宣称关闭该验收项。
