...
ALPHA-05 仍负责浏览器必需检查决策与远端生效。注册表发布和试用部署属于独立动作。发布前须重新核对注册表命名空间／版本与发行策略；只有分发决策完成后才能发布这份准确的已审查字节。之后，干净消费者须安装准确注册表版本，验证完整性／构建身份并重跑升级门槛。本地 tarball 安装成功不能关闭注册表消费门槛。ALPHA-04 不发布 Rust crate、发行标签或托管试用。
