...
No JSON field, null/omission rule, enum spelling, source/plan version or exit-code policy is changed in PR-012. Tests pin relevant semantic fields rather than volatile adapter/timing snapshots. Current data is verified locally; packaged consumption, device-loss/OOM classification, stale-result scheduling, full compatibility/release policy and deferred remote platform CI remain their later M4 gates.
