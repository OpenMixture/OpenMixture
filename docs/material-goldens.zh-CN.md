...
每个通道声明编码及连接／默认来源。每个用例为四个通道提供结构检查。默认用例没有因果规则；每个变体均为四个通道指定规则，包括保持不变的通道，且至少声明一个有意义的变化。未知字段、非法阈值、缺少通道、重复 ID、奇数／欠采样／不能整除的棋盘格数量、错误驱动版本均在渲染前失败。此 schema 将材质 golden 固定为 1K；PR-010 通过 `trace-2k` 单独提供代表性 2K 执行证据。
