...
## M6A-05 acceptance scope decision — 2026-09-21

The [retained qualification](./evidence/m6a-05/README.md) explicitly limits M6-A milestone closure to the recorded Linux pinned Native SwiftShader / Chromium software matrix. The maximum component delta ≤1 remains mandatory on every claimed qualified pair, with unchanged frozen inputs; it is not a universal hardware guarantee. This is an explicit scope revision based on the preserved Windows hardware failure (normal delta up to 8), not a passing verdict for that run or a tolerance increase. Hardware expansion requires a separate numerical/compatibility decision and passing resource, Scalar and material regressions. No existing shader or golden changes. M6A-02–04 are implemented; `cargo xtask test-node image-input` is now available. Earlier future-tense design statements above describe the original M6A-01 checkpoint. Publication, dependency-PR integration and maintainer visual approval are distinct from machine qualification and agent visual review.
