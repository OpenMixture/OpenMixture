...
PR-008 introduced protected material comparisons and the [glazed ceramic fixture](../fixtures/materials/glazed-ceramic/README.md). It uses the existing CLI to validate, compile, and render all cases through `wgpu`; tooling measures the returned PNGs. That batch added no nodes, shaders, runtime APIs, or pixel executors. Remote CI was deferred at PR-008; its local pass did not close M0/M1/M2 or M3. Later [remote CI acceptance](./evidence/remote-ci/README.md) and the [release record](./release.md) define the accepted matrix and current milestone status.
...
Leather detail min/default/max and coarser grainScale must satisfy image goldens, spatial, normal and causality constraints together. Controlled PBR views consume actual PNGs, applying the height slope through the exported normal once with no second bump. See [leather appearance review](../fixtures/materials/leather/review/README.md). Ceramic has user acceptance; leather human review is accepted. PR-010 adds directional wood and 2K evidence below; wood human acceptance is recorded. Remote CI was open at that batch and passed in the later linked acceptance.

## PR-010 directionality and 2K evidence

The [wood fixture](../fixtures/materials/wood/README.md) adds `directional`: all `spatial` gates plus a declared horizontal/vertical grain axis and minimum cross-grain / along-grain gradient energy ratio. Energy is mean squared adjacent red difference including wrap. The denominator is floored at one byte squared, so constant or almost-constant fields cannot pass using an infinite ratio. `minEnergyRatio` must exceed one. Literal tests transpose an image without changing its histogram and reject the wrong axis, scrambling, and flat pixels. Wood additionally requires height/normal sign agreement and variant causality. This is an additive repository acceptance contract; `.mix` v1 is unchanged.

`cargo xtask golden check` now covers ceramic, leather and wood at 1024. `cargo xtask trace-2k` ranks all eleven material cases at 2048 by estimated peak bytes and measures the largest under the same explicit adapter policy. It retains separate inputs, selection, doctor, render and descriptor-allocation evidence, verifies the 512 MiB peak budget and unchanged 1K baselines, and never updates goldens. See [trace semantics](./development.md). Human review remains independent of these machine gates; the later M4 acceptance is recorded separately in the release record.
