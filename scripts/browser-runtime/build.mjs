...
digest.update(JSON.stringify({ compilerVersion, bindgen: toolVersion, compilationEnvironment }));
digest.update('\0');
...
await cp(join(root, 'packages/runtime'), staging, { recursive: true });
await rm(join(staging, 'test'), { recursive: true, force: true });
...
await writeFile(join(staging, 'src/build-info.mjs'), `export const buildInfo = Object.freeze(${JSON.stringify(buildInfo)});\n`);
...
await writeFile(join(destination, 'receipt.json'), JSON.stringify({ ...buildInfo, tarball: relative(root, archive), sha256,
  toolchain: { rust: command('rustc', ['--version']), bindgen: toolVersion, node: process.version, npm: command('npm', ['--version']), target: 'wasm32-unknown-unknown' },
