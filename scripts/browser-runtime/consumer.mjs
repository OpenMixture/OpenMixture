...
export function assertBrowserReport(report, mode = 'registry') {
  ...
  assert.equal(report.stats.expected, mode === 'candidate' ? 20 : 13, 'all public consumer tests must execute');
...
export async function qualify(mode, packageDirectory, output) {
  ...
  const env = { ...process.env, npm_config_cache: join(staging, '.npm-cache'), MIXTURE_RESOURCE_TESTS: '1', MIXTURE_REUSE_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_ASSET_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_SUBTRACT_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_MORPHOLOGY_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_BRICK_TESTS: mode === 'candidate' ? '1' : '0' };
