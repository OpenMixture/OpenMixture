...
export function candidateManifests(manifest, lock, version, bytes) {
  ...
  assert.equal(version, '0.8.0-alpha.0', 'review the candidate compatibility contract before upgrading');
...
export function assertBrowserReport(report, mode = 'registry') {
  ...
  assert.equal(report.stats.expected, mode === 'candidate' ? 21 : 13, 'all public consumer tests must execute');
...
export async function qualify(mode, packageDirectory, output) {
  ...
  const env = { ...process.env, npm_config_cache: join(staging, '.npm-cache'), MIXTURE_RESOURCE_TESTS: '1', MIXTURE_PAINTED_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_REUSE_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_ASSET_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_SUBTRACT_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_MORPHOLOGY_TESTS: mode === 'candidate' ? '1' : '0', MIXTURE_BRICK_TESTS: mode === 'candidate' ? '1' : '0' };
  ...
  try {
    ...
    if (mode === 'candidate') {
      ...
      const reuseSource = join(root, 'docs/evidence/perf-mat-before/material.mix');
      record.reuseSourceSha256 = hash(await readFile(reuseSource));
      await cp(reuseSource, join(staging, 'public/reuse-material.mix'));
      await writePaintedMetalRequests(join(staging, 'public/painted-metal'));
