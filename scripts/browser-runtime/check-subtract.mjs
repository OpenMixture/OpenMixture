// Exact public Native/browser subtract comparison; neither adapter implements pixels.
import assert from 'node:assert/strict';
import { readFile, readdir, mkdir, writeFile, copyFile } from 'node:fs/promises';
import { join, resolve } from 'node:path';
import { execFileSync, spawnSync } from 'node:child_process';
const args = process.argv.slice(2);
assert.equal(args.length, 2, 'usage: check-subtract.mjs <candidate-qualification> <fresh-output>');
...
async function collect(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if (entry.name === 'subtract-evidence.json') matches.push(path);
  }
}
...
assert.equal(browser.rows.length, 64);
...
const result = spawnSync('cargo', ['test', '--manifest-path', 'examples/native-consumer/Cargo.toml', '--locked', '--all-features', '--target-dir', 'target/native-consumer', '--test', 'subtract', '--', '--ignored', '--nocapture'], { encoding: 'utf8', env: { ...process.env, MIXTURE_SUBTRACT_BROWSER: browserPath, MIXTURE_SUBTRACT_EVIDENCE: nativePath } });
await writeFile(join(output, 'stdout.log'), result.stdout ?? '');
await writeFile(join(output, 'stderr.log'), result.stderr ?? '');
if (result.error) throw result.error;
assert.equal(result.status, 0, 'subtract Native/browser comparison failed; inspect retained output');
...
await writeFile(join(output, 'comparison.json'), JSON.stringify({ ok: true, cases: 64, maxComponentDelta: 0, sourceRevision: receipt.consumerRevision, browserBuild: receipt.build, nativeAdapter: native.adapter }, null, 2) + '\n');
console.log(`Subtraction Native/browser comparison passed: ${output}`);
