...
**历史记录：** `PR-001` 至 `PR-010` 是已完成的实施批次标识，不是 GitHub PR 编号。下文保留原始顺序、验收要求及各阶段状态作为历史记录。当前验收状态见[路线图](./ROADMAP.zh-CN.md)和[发布状态](./docs/release.zh-CN.md)；新工作按照[治理规则](./docs/governance.zh-CN.md)使用真实 GitHub PR。
