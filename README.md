...
**Current status (2026-09-21):** ENG-01–04 are implemented. The npm Alpha `@openmixture/runtime@0.2.0-alpha.0` is published with Scalar composition; its [release record](./docs/evidence/npm-020-alpha/README.md) binds the exact archive and qualification. Native M4/M4.1, bounded M5 and earlier browser/product acceptance retain their original scope. Rust crates remain unpublished; both browser checks are required on main. The [Post-Alpha roadmap](./ROADMAP.md) owns current work, and [Alpha closeout](./docs/browser-alpha.md) retains first-delivery evidence.

[M6A-02](./docs/m6a-02-core-resources.md) adds the thirteenth Core node contract, image-input, immutable resource preparation and plan v2; [M6A-03](./docs/m6a-03-native-resources.md) adds Native image upload and execution. Rust source is 0.3.0 and the browser candidate is 0.3.0-alpha.0/API schema 2, both unpublished; the exact published npm version remains 0.2.0-alpha.0.
