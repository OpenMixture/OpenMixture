...
PR-015 adds local archive verification in repository tooling only. The independent consumer is staged with normalized public package contents in a disposable external workspace; local version patches resolve those extracted crates, never producer-private modules. Package licenses/README/unit assets are self-contained, committed locks remain unchanged and publication remains disabled. The [compatibility record](./docs/compatibility.md) consolidates existing boundaries; [M4 release status](./docs/release.md) retains deferred external gates.
